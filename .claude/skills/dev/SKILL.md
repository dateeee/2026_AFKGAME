---
name: dev
description: AFK GAME の機能をフロントエンド（Vue 3）とバックエンド（FastAPI）で実装する。「戦闘システムを実装して」「装備の売却機能を追加して」など、仕様書に基づく新規実装・機能追加の依頼で使用する。仕様書の読み込み順、Phase制約、TDDでの実装順序（テストリスト→モデル→スキーマ→サービス→ルーター→フロント）、技術規約（SQLAlchemy 2.0 / Pydantic v2 / Pinia Setup Store）を含む。
---
...
**バックエンドは TDD（テスト駆動開発）で実装する**（開発工程定義書 §3.4〜§3.5）。テストを先に書き、それを満たす実装を後から書く。フロントエンドは対象外。
...
### テストリスト（分岐観点）
- （詳細設計の分岐一覧から導出した観点を列挙。これが最初に書くテストになる）

...
カバレッジを通すためだけに `pragma: no cover` を付けることは**禁止**。現在このリポジトリでの使用は **0件**。

## 5. 現在の整備状況

**単体テストゲート通過（2026-08-02）**: `app/` 全40モジュールが C1 100%、306件 PASS、1,578 stmts / 296 branches。

| # | ルール |
|---|-------|
| 1 | 数値は `pytest` 実行時に更新する。製造の追加・変更時は**同一変更内で100%を維持**する |
| 2 | 実装済み（Phase 1〜2）のテストは遡及整備で C1 100% に到達済みのため**書き直さない** |
| 3 | 本工程で補完した分岐は、[detail-design.md](detail-design.md) §4 の分岐一覧へ**逆反映**する |
| 4 | 検出した実装の疑義（仕様乖離・デッドコード等）は `docs/known_issues.md` へ記録する |

...
| コードとの整合性 | ER図↔`afkgame-domain` の Entity/Repository（定義書と併せた**三者一致**。判定は [basic-design.md](basic-design.md) §4） / クラス図↔`afkgame-domain` の Service・`stores/` / 画面遷移図↔`frontend/src/router/` / APIシーケンス図↔`afkgame-web` の `@RestController`・`frontend/src/api/` |

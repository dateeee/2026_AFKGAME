...
| # | 判断 |
|---|------|
| 0 | **書く前に測る**。①対象の `--sections <path>` ②[carryover_notes.md](../../docs/backlog/carryover_notes.md) の該当行（先行セッションが残した残量メモ）③表へ行を足すなら**該当 H2 の残量 ÷ 既存1行の字数**で入る行数。残量が上限の5%未満、または行が入らないなら**追記せず #2 へ** |

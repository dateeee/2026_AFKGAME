...
**測ってから決める**。圧縮を小刻みに試して測り直すと、往復した末に結局分割することになる。

| # | 判断 |
|---|------|
| 0 | **書く前に測る**。①対象の `--sections <path>` ②[carryover_notes.md](../../docs/backlog/carryover_notes.md) の該当行（先行セッションが残した残量メモ）③表へ行を足すなら**該当 H2 の残量 ÷ 既存1行の字数**で入る行数。残量が上限の5%未満、または行が入らないなら**追記せず #2 へ**。`--sections` が `!`（H2超過）を出しているファイルは、残量が追記予定字数を下回るなら圧縮ではなく**分割を既定**にする（圧縮で捻出できるのは数百字で、H2超過は解消しない） |
| 1 | 着手前に `--sections <path>` で**塊（H2）ごとの字数を一括計測**する。試すたびに測り直さない |
| 2 | **超過幅が300字超、または是正後の残量が上限の5%未満になる見込みなら、圧縮を試さず分割・移管から入る**（圧縮しきっても実用的な余白が残らず、次の追記で再発する）。**圧縮で捻出してよいのは既存記述の改稿ぶんだけ**で、新規の節・行を入れる余白を圧縮で作らない |
| 3 | それ未満は圧縮でよい（表化・重複排除・冗長な前文の削除）。**圧縮の予定字数も `len()` で実測**してから編集する（[profile.md](profile.md) §7 規約7） |
| 4 | **残量300字未満のファイルで圧縮と追記を同時に行うときは、`len()` の差し引きで確定させない**。複数箇所の増減は誤差が積み上がる（実績: net で足りる見積もりが実測51字超過）。編集後に `--sections` を再実行して確かめる |
| 5 | [worktree_guide.md](../../docs/process/worktree_guide.md) §3 のホットスポット（changelog・efficiency_memo・carryover_notes）を**全面書き換え**するときは、調査と個別修正を先に終えてから対象を読む。読了後に長い作業を挟んだら `git log -1 -- <path>` で更新の有無を確かめる（別セッションの追記で読み直しになる） |
...
**索引に一覧表を新設するときは通し番号で参照させず、「どの分冊が正か」だけを持たせる**（番号は分冊内の連番にする）。横断の通し番号は1件の増減で全参照元の追随を強いる。

区分A・Dは台帳に載らないため移管で余裕を作る: `CLAUDE.md` → 詳細を `docs/**` の正へ移しリンク参照、`.claude/project/profile.md` → 工程別ファイル `.claude/project/<スキル名>.md`（本ファイルがその形）。

**ファイル移動を伴う再編の事前調査**は、影響するリンクを**両方向**数える。片方だけで計画を組むと、移動後に `check_docs.py` で大量の違反が出てから作り直しになる。

| 方向 | 対象 | 変わる理由 |
|------|------|----------|
| 入る | 外部 → 移動元を指すリンク | 参照先のパスが変わる |
| 出る | **移動するファイル自身** → 外部を指すリンク | 移動先からの**相対深さ**が変わる（`../docs/tech/…` → `../tech/…`） |
...
| 参照の張り替え先が区分A・D | **張り替え前に残量を測る**。同義でより短い表記（`tech_api.md` → `tech_api/`）を選び、増分を0以下に抑える |

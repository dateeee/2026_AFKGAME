...
> **コードを書く前に** [coding-standards-backend.md](../references/coding-standards-backend.md)（要約）と、正の規約から**共通ぶん + 書く層の分冊**を読む。どれを読むかは [coding_standards_backend.md](../../docs/process/coding_standards_backend.md) §2「読む順」、クラスの置き場・呼び出し方向は同 `layering.md`。ベースは TERASOLUNA 開発ガイドライン 5.11.0.RELEASE 日本語版（差分は各分冊が持つ）。

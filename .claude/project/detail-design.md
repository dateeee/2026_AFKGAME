...
旧形式（3列）のセクションが残っているファイルは、**改稿するタイミングで標準形式へ移行**する。
...
| 6 | 同一条件の真・偽の行には**同じ分岐点名**を使う（`check_branch_list.py` が対として検証する） |
| 7 | 記載後に `python scripts/check_branch_list.py` を実行し ERROR を解消する。「1行のみ」の WARN は例外経路であることを確認する |

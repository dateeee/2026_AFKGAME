...
**書き始める前に割る**: 分岐が合計50件を超える見込み、または対象APIが3つ以上なら、1ファイルで書かず索引 + 操作別ファイルへ分ける（書き切ってから上限超過に気づくと全面的な書き直しになる）。
...
| 7 | **数値・範囲・選択肢を書く前に [spec_ownership.md](../../docs/process/spec_ownership.md) の検出パターン列を grep する**。そのトピックの正でないファイルへは再掲せずリンクを置く（`check_docs.py` が「正の逸脱」で ERROR にする） |
| 8 | **更新を伴う手順には「同時実行で判定を通り抜けた場合」の行を立てる**。「読んで判定 → 条件なしで更新」は READ COMMITTED で担保が抜ける（backend-review ISSUE-901・902）。更新側の条件と件数判定で締める振る舞いを書く。C1 では拾えない（分岐として存在しない）ため本工程で持つ |

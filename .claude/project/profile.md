...
| 6 | 大きな出力（ログ・テスト結果・git履歴・集計・検索）の処理は context-mode（`ctx_batch_execute` / `ctx_execute`）で行い、生出力を会話に持ち込まない。ファイルの分析・要約は `ctx_execute_file`。`Read` の全文読みは Edit 前提のときのみ。**逆に Edit 前提なら必ず `Read` で読む**（`cat`・`ctx_execute_file` で読んだファイルは Edit が通らない。片側だけ守ると往復が増える） |
...
| # | 規約 |
|---|------|
| 1 | 文字数上限は [docs/process/documentation_rules.md](../../docs/process/documentation_rules.md) §3（`.claude/**` は区分D = 6,000字） |

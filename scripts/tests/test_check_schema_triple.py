...
定義書 / ER図 / Java Entity / Flyway DDL を `tmp_path` に最小構成で作り、`ROOT` 系の定数を
差し替えて検証する。各検査は「全ソースが一致した状態を通す（緑パス）」と
「1か所だけ崩すと検出する（変異）」を対で置く。

DDL 照合は `Sources.ddl` が空なら丸ごと飛ばす。そのため既定の `sources()` は DDL を
載せず、DDL の検査は `sources_with_ddl()` を使う。

Entity は列を持たない POJO のためファイルの実在だけを見る（本体の docstring を参照）。
"""
...
def def_doc(rows, table="players", phase="Phase 1", cls="Player",
            impl=True, planned=False, uniques=()):
    parts = [f"## 1. `{table}`（{phase}）", ""]
    if impl:
        head = "実装予定" if planned else "実装"
        parts += [f"{head}: `com.afkgame.domain.model.{cls}`", ""]
    for line in uniques:
        parts += [f"一意制約: {line}", ""]
...
def entity_doc(cls="Player"):
    """Entity は実在だけを見るため、中身は最小の POJO でよい。"""
    return f"package com.afkgame.domain.model;\n\npublic class {cls} {{\n}}\n"
...
def repo(tmp_path, monkeypatch):
    ...
    entity = (tmp_path / "backend" / "afkgame-domain" / "src" / "main" / "java"
              / "com" / "afkgame" / "domain" / "model")
    ddl = tmp_path / "backend" / "afkgame-initdb" / "src" / "main" / "resources" / "db" / "migration"
    for d in (defs, er, entity, ddl):
    ...
    monkeypatch.setattr(mod, "ENTITY_DIR", entity)
    monkeypatch.setattr(mod, "DDL_DIR", ddl)

    r = SimpleNamespace(root=tmp_path, def_dir=defs, er_dir=er, entity_dir=entity,
    ...
    r.write_entity = lambda cls="Player": (entity / f"{cls}.java").write_text(
        entity_doc(cls), encoding="utf-8")
...
def sources() -> mod.Sources:
    """DDL を載せない Sources（定義書 ↔ ER図 ↔ Entity 実在の照合を見るテスト向け）。"""
...
def test_parse_definitions_reads_table_columns_and_impl(repo):
    ...
    assert (d.cls, d.planned) == ("Player", False)
    assert [c.name for c in d.columns] == ["id", "name"]
    assert d.columns[0].pk is True and d.columns[1].unique is True


def test_parse_definitions_marks_planned_entity(repo):
    repo.write_def(def_doc(DEF_ROWS, planned=True))
    d = mod.parse_definitions()["players"]
    assert (d.cls, d.planned) == ("Player", True)
...
def test_entity_exists_finds_written_class(repo):
    assert mod.entity_exists("Player") is True


def test_entity_exists_returns_false_for_absent_class(repo):
    assert mod.entity_exists("Ghost") is False
...
def test_check_columns_detects_declared_entity_that_does_not_exist(repo):
    (repo.entity_dir / "Player.java").unlink()
    errors = mod.check_columns(sources())
    assert any("`com.afkgame.domain.model.Player` が無い" in e for e in errors)


def test_check_columns_allows_absent_entity_when_marked_planned(repo):
    repo.write_def(def_doc(DEF_ROWS, planned=True))
    (repo.entity_dir / "Player.java").unlink()
    assert mod.check_columns(sources()) == []


def test_check_columns_detects_planned_entity_that_already_exists(repo):
    repo.write_def(def_doc(DEF_ROWS, planned=True))
    errors = mod.check_columns(sources())
    assert any("「実装予定:」だが `Player.java` が実在する" in e for e in errors)
...
def test_check_unique_passes_when_ddl_is_not_loaded(repo):
    composite_repo(repo)
...
def nofk_repo(repo, er_attr="int item_id"):
    repo.write_def(def_doc(["| `id` | INTEGER | 不可 | — | PK |",
...
def test_check_nullable_passes_when_definition_and_er_agree(repo):
    nullable_repo(repo)
...
def test_check_columns_allows_missing_ddl_table_when_marked_unimplemented(repo):
    repo.write_def(def_doc(DEF_ROWS, phase="Phase 4・未実装", planned=True))
    (repo.entity_dir / "Player.java").unlink()
    (repo.ddl_dir / "V1__initial.sql").unlink()
...
def test_main_returns_zero_and_reports_scale(repo, monkeypatch, capsys):
    ...
    assert "定義書 1 テーブル / 2 列" in out and "Entity 1 件（実装宣言）" in out
    assert "差分なし" in out
...
def test_main_lists_unimplemented_tables(repo, monkeypatch, capsys):
    ...
    monkeypatch.setattr(mod.sys, "argv", ["check_schema_triple.py", "--summary"])
    mod.main()
    assert "DDL 照合を除外（定義書が未実装と明記） 1 件: players" in capsys.readouterr().out

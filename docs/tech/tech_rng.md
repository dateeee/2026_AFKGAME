...
| 項目 | 仕様 |
|------|------|
| 実体 | `random.Random` の**インスタンス**を1つ生成し、tick処理の入口から引数で受け渡す |
| グローバル `random.*` の直呼び | **禁止**（テストでのシード固定がプロセス全体に波及し、並行リクエスト間で干渉するため） |
| 生成単位 | 1リクエスト（1回の `/api/battle/tick`）につき1インスタンス |
| シード | 通常は未指定（OSエントロピー）。調査時のみ環境変数 `BATTLE_RNG_SEED` で固定できる |
| 永続化 | RNGの内部状態はDBに保存しない（リクエスト間で持ち越さない） |
| 適用範囲 | 戦闘・ドロップ・装備生成・スカウトを含む全サービスが同一インスタンスを受け取る（乱数源を1本化する） |

- インスタンス注入により、単体テストは `Random(42)` を渡すだけで完全に決定的になる（`monkeypatch` でのグローバル差し替えが不要）
- 特定分岐の網羅には、`random()` の戻り値を列で与えるスタブ（`FakeRandom([0.04, 0.99, ...])`）を用いる

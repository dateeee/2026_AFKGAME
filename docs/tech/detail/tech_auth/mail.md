...
| 項目 | 規約 |
|------|------|
| 実行位置 | **呼び出し元のトランザクションがコミットしたあと、トランザクションの外**で実行する。送信要求はトランザクション内で登録し、コミット成功を待ってから送る |
| ロールバック時 | 送信しない（存在しないユーザー・トークンに宛てたメールを出さないため） |
| 失敗の扱い | 業務エラーにしない。WARN ログだけを残し、API 応答・HTTPステータスを変えない |
| 再試行 | しない。1回の送信要求につき送信は1回 |
| 同期・非同期 | 同期で送り、応答時間に含める（非同期化しない）。接続・応答のタイムアウトは**5秒**で、超過は送信失敗とする |
| ログ | START / END を通信ログへ出す。**トークン生値・本文をログへ出さない**。宛先は `LogKey.EMAIL` へ渡してマスクする |

### 16.2 設定値

値は `afkgame-env` の設定保持 Bean から読む（`tech_operations.md` §12.2）。
...
| タイムアウト | `mail.smtp.timeout` | —（環境変数で上書きしない） | `5000`（ミリ秒） |
| 差出人 | `mail.from` | —（同上） | `noreply@afkgame.example`（**仮置き**。運用ドメイン確定時に差し替える） |
| リンク生成元 | `frontend.base.url` | `FRONTEND_BASE_URL` | `http://localhost:5173` |

**`SMTP_HOST` が未設定のときは送信を行わず、宛先と用途を INFO ログへ残すだけにする**（ローカル開発・テストで SMTP サーバーを立てずに動かすため）。本番プロファイルでの未設定は起動時バリデーションで落とす（`tech_operations.md` §12.2）。

### 16.3 メール本文

本文は**プレーンテキストのみ**とし、HTMLメールを送らない。トークンの生値は本文のリンクにだけ載せる。

| 種別 | 件名 | リンク | 有効期限 |
|------|------|-------|---------|
| 確認メール | `【AFK GAME】メールアドレスの確認` | `{frontend.base.url}/verify-email?token=<生値>` | 24時間（`tech_auth.md` §3 が正） |
| 再設定メール | `【AFK GAME】パスワードの再設定` | `{frontend.base.url}/password-reset?token=<生値>` | **1時間**（本節が正。確認メールより短くするのは、乗っ取り時に書き換えられる窓を狭めるため） |

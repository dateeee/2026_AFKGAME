...
- 選択・状態は色だけで示さず、**帯・アイコン・文言**を併用する（`systems/ui.md` 色非依存）
...
| `AppHeader` | ブランド（ホームへ戻る）・ゴールド・設定・お知らせ（Phase 3〜）。**ゴールドの表示場所はここ1箇所に固定する**。要素の一覧は `systems/ui.md` ヘッダ表が正 |
...
- 画面追加は `navItems.ts` に1行追加すれば両ナビに反映される。**5項目を超えたら**モバイルは1項目64px を割るため末尾を「その他」へまとめる（PCは全項目を並べる）。対象項目は `systems/ui.md` ナビゲーション構造が正

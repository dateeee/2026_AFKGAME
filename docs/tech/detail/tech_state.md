...
## 5. 分岐一覧（単体テスト観点・Phase 1）

塔操作（select・retire・mode）と階進行（目標階クリア・HP閾値撤退・上限追従）の分岐は [tech_tower.md §0](tech_tower.md) の各分冊が持つ。本表は状態機械そのもの（探索セッション・不変条件）のうち、**Phase 1 の実装だけで成立する**分岐。装備の没収と操作ガードは §6。

| # | 分岐点 | 条件 | 期待する振る舞い |
|---|-------|------|----------------|
| 1 | 全滅ペナルティ | `runGold = 0` で全滅 | ゴールド減算0・下限を割らない |
| 2 | 全滅ペナルティ | EXP=0 のキャラで全滅 | EXP減算0 |
| 3 | 全滅ペナルティ | 全滅後のHP | 全員 `maxHP` へ全回復 |
| 4 | 不変条件 | 不変条件（§1.1）に違反するデータを読んだ | `500 INTERNAL_UNEXPECTED_ERROR` + ERRORログ（黙って補正しない） |

> WARN許容 #4: 真偽の対を持たない例外経路（不変条件違反は §1.1 のとおり検出のみで、正常側は全分岐の前提として常に検証される）。#1〜#3 はペナルティの境界を1観点ずつ試験する行で、適用順そのものの網羅は §3 の6手順を通しで検証するテストが担う。

## 6. 分岐一覧（単体テスト観点・Phase 2〜3）

Phase 1 では成立しない分岐。装備（`runEquipmentIds`）とパーティ編成が未実装で、`players` に対応する列も持たないため。**展開はその機能を載せる Phase のテストリスト作成が担う**（本表を参照するテストはまだ無く、`check_branch_list.py` の照合対象外）。

| # | 分岐点 | 条件 | 期待する振る舞い |
|---|-------|------|----------------|
| 1 | 編成ロック | `EXPLORING` / `IN_BATTLE` で `party/edit` | `400 PARTY_LOCKED_IN_TOWER` |
| 2 | 編成ロック | `IDLE` で `party/edit` | 変更できる（§4） |
| 3 | 全滅ペナルティ | 装備中の装備をrun中に取得して全滅 | 装備解除のうえ削除 |

> WARN許容 #3: 真偽の対を持たない行（没収しない側は §3 の「リタイア・HP閾値撤退・目標階クリア」＝確定してリセットが受け持つ）。

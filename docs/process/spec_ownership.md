...
| ログ3種別（通信・アプリケーション・エラー）の定義・出力先ファイル・ローテーション・出力主体・共通部品の使い方 | `docs/process/coding_standards_backend/logging.md` | `docs/tech/basic/tech_logging.md`, `docs/tech/nonfunctional/tech_operations.md` | `communication\.log` | 形式・項目名・ロガー名体系・`reason`・エラーコード体系の正は `tech_logging.md` に残す（語彙 = 仕様 / 出力先と書き方 = 規約 で分けた。2026-08-09 確定） |

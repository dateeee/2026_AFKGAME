...
| 完了基準 | game_spec.md 系へ反映され、仕様確定ゲート（`development_process.md` §4）を通過している |
...
| 成果物 | `tech_spec.md` 配下の**処理**（battle / offline / tick / polling）と**横断規約**（rng / numeric / state）、**数値**は [master_data.md](../data/master_data.md)（索引）＋ [data/](../data/) 配下 |
...
| 対象 | **バックエンドのみ**（`afkgame-domain` の `service/`・`masterdata/` は厳格に、`afkgame-web` の `api/` は MockMvc で先行作成）。フロントエンドは対象外 |
| 入力 | 詳細設計の**分岐一覧（単体テスト観点）**（§3.3 完了基準） |
| 主な作業 | 分岐一覧を1観点1テストへ展開し、各モジュールの `src/test/java/` に**失敗するテスト**として記述する |
...
| 対象 | バックエンド（`afkgame-domain` の `service/`・`masterdata/`、`afkgame-web` の `api/` のメソッド・分岐） |
| フレームワーク | JUnit 5 + Mockito + JaCoCo |
| 配置 | 各モジュールの `src/test/java/`（対象クラスと同じパッケージ） |
| カバレッジ基準 | **C1（分岐網羅）100%**: JaCoCo branch カバレッジ100%（`mvn test jacoco:check`） |
| 除外規則 | JaCoCo の `<excludes>` 指定は理由コメント必須（分岐を持たない起動・設定クラスのみ許容）。現時点の指定は0件 |

...
| [web.md](coding_standards_backend/web.md) | `afkgame-web`: §1 責務 / §2 コントローラ / §3 Resource / §4 設定・フィルタ / §5 エラー応答（→ `exception.md`） / §6 命名 / §7 セキュリティ / §8 REST API 設計（5.1）との差分 | Web層を書くとき・URI やステータスを決めるとき |
...
`common.md` →（置き場・呼び出し方向に迷えば `layering.md`）→ いま書く層の分冊（エラー処理を書くなら層を問わず `exception.md`、ログを書くなら層を問わず `logging.md`（索引 → 種別ごとの分冊）、横断処理なら `filter.md` → 必要なら `interceptor.md`）。分冊にもガイドラインにも無い判断は近傍の既存コードに倣う（[basis.md](coding_standards_backend/basis.md) §2 #1）。

...
`common.md` →（置き場・呼び出し方向に迷えば `layering.md`）→ いま書く層の分冊（エラー処理を書くなら層を問わず `exception.md`）。分冊にもガイドラインにも無い判断は近傍の既存コードに倣う（[basis.md](coding_standards_backend/basis.md) §2 #1）。

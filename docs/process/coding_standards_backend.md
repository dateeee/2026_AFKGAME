...
- `afkgame-domain` に Web 層の型（Spring MVC・`jakarta.servlet`・`HttpStatus`）を持ち込まない。HTTP ステータスは `int` で保持する（`AppException` がその例）
...
| Mapper | `<Entity>Mapper`。XML は**同名・同パッケージのリソース配下** | `UserMapper.java` ↔ 同パッケージの `UserMapper.xml` |
...
- **JSON のフィールド名は lowerCamelCase**。Jackson の既定でそのまま出るため `@JsonProperty` で改名しない（[tech_api_common.md](../tech/basic/tech_api_common.md) §5.0）
...
| 層 | 規約 |
|----|------|
| コントローラ（`api`） | マッピング・入力検証・Resource 変換**だけ**を書く。業務分岐・計算・DB アクセスを置かない。ボディは `@Valid @RequestBody`、戻り値は Resource（`ResponseEntity` はステータス・ヘッダを変える場合のみ） |
...
- 現在時刻・乱数は「外から受ける」。乱数は `RandomFactory` から取得して引数で引き回す（[tech_rng.md](../tech/detail/tech_rng.md) §2）。静的な共有インスタンスを持たない
  **暗号用途の `SecureRandom` だけは例外**（スレッドセーフのため `private static final` で共有可。`RandomFactory` をトークン生成に使うと予測可能になる）
...
| 3 | 検査例外を新設しない（`RuntimeException` 派生）。ライブラリの検査例外は捕捉して `AppException` か `IllegalStateException` へ変換する |
...
| 2 | クラス Javadoc に**仕様書の参照先**を `<p>仕様: docs/....md §N` の形で書く（実装の根拠を追えるように） |
...
| 6 | 行コメントは「何を」ではなく「**なぜ**」を書く。コードを読めば分かることを繰り返さない |

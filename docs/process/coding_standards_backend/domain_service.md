...
| # | 規約 |
|---|------|
| 1 | `@Service` を付けた**具象クラスのみ**を作り、Service インタフェースを設けない（ガイドライン 3.2.5.4.1 との差分）。インタフェースを推奨する理由は AOP のプロキシ方式とスタブ化だが、Spring Boot は CGLIB プロキシで足り、テストは Mockito が具象クラスをモックできるため |

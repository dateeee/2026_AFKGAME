...
| 3 | 可変フィールド + getter/setter を許すのは Entity だけ（`common.md` §5 #4・#5）。MyBatis がマッピングするため |
...
Repository は Service へ Entity のライフサイクル操作を提供するドメイン層のインタフェースで、実装は MyBatis3 が Mapper インタフェースの仕組みで生成する。書くのは**インタフェースとマッピング XML の2つだけ**（RepositoryImpl は書かない。`layering.md` §3）。
...
| 8 | Repository から Service を呼ばない（依存の逆流。`common.md` §2） |

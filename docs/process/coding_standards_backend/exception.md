...
| `SystemException`（gfw） | `SystemException`（`domain.exception`・自作） | `terasoluna-gfw` が未導入のため（`basis.md` §1）。導入後に gfw のクラスへ寄せるかは移行 STEP 2R-A で判断する |
...
| # | 規約 |
|---|------|
| 1 | ビジネスルールの違反は**ロジックで検知して** `AppException` を投げる。コードは仕様の値をそのまま使い、`@throws` にコードを書く（`common.md` §8 #3） |
...
| 3 | ガイドラインの `ExceptionLogger`・`ExceptionLevelResolver`（4.3.5.1）は**使わない**。`terasoluna-gfw` が未導入であることに加え、出力先とレベルを機能単位で切り替えるためロガー名体系（`common.md` §7 #1）で出す方針のため。例外コードの先頭1文字でレベルを決める仕組み（`DefaultExceptionLevelResolver`）も採らず、レベルは本節の分類で決める |

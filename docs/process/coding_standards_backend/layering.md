...
| Service | 業務処理の提供とトランザクション境界の宣言。**Form・`HttpServletRequest` など Web の情報を扱わない** | `domain.service.<領域>` の `<領域>Service`（インタフェース）+ `<領域>ServiceImpl`（[domain/service.md](domain/service.md) §3 #1） |

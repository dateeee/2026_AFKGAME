...
- **`docs/backlog/java_migration.md` は分割済み**（2026-08-09）。索引（前提・STEP 一覧表 = 進捗の正）+ `java_migration/{tech_selection,steps,changes}.md`。**進捗と STEP の追記先は `java_migration/steps.md`**（余裕約3,700字）。索引側は表の「状態」列だけを更新する
...
- **既存実装から引き継ぐ流儀（2R でコードを作り直しても内容は有効）**: ①マッピング XML の `resultMap` は明示するが、理由に「自動変換と食い違うため」と書かない（規約は変換を `map-underscore-to-camel-case` に任せる方針で、実際に食い違うのは `is_guest` ↔ `guest` のような boolean getter だけ。`UserRepository.xml` のコメントのみが正しい）②マスターデータのレジストリ4件は `(loader, resourcePath)` のパッケージプライベートなコンストラクタと `contains()` を持つ形にそろっている。5件目もこれに倣う（異常系フィクスチャは domain 側 `masterdata-invalid/` に置き、web 側の統合テストはコンテキスト起動失敗の検証だけに使う）③サービスは時刻を `Clock` で受け取る（ISSUE-605 案A。`afkgame-env` の `TimeConfig` が Bean 化）④`PlayerInitializationService.initialize()` は `Propagation.MANDATORY` で、register からも `@Transactional` 配下で呼ぶ（外から呼ぶと `IllegalTransactionStateException`）⑤`APP_ENV` は必須で既定値を持たない（統合テストには `@ActiveProfiles("local")` が要る）——⑤の与え方だけは 2R の設定方式変更で置き換わる
- **データアクセスは Repository で抽象化する**（2026-08-09 決着。正は `coding_standards_backend/layering.md` §3・`domain.md` §3）。Repository インタフェースとマッピング XML はフレームワーク非依存なので **2R の再構築を跨いでそのまま持ち越す**（Boot に依存するのはテスト側だけ）。作成単位は主体 Entity で、`player_settings`・`inventory_items` は `PlayerRepository`、`character_equip_slots` は `CharacterRepository` が持つ。`refresh_tokens` はトークンハッシュで独立に引くため主体扱い。従 Entity を含む Repository のテストは `@Nested` で観点を分ける（`PlayerRepositoryTest`）。従 Entity 分だけをモックできなくなるので、統合テストで一部だけ失敗させたいときは `@MockitoSpyBean`（`GuestInitializationRollbackIntegrationTest` が実例）
...
- **素の Spring は Boot の緩い形式を解さない**（2026-08-09。2R-C）。`@Value` で `Duration` を受けるには ISO-8601（`PT30M`）が要るため、設定は**分・日の整数**で持ち Java 側で `Duration.ofMinutes(...)` へ組む。カンマ区切りは `String[]` で受けて `List.of(...)`、空文字は `Long` へ null 変換される

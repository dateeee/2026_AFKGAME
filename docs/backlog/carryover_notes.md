...
- **認証の入力長・メール正規化は仕様側を確定済み（2026-08-09・STEP 3-A-3）だが実装が未追随**。①メール長は **254**（`tech_auth/account.md` §9「入力長」が正。`tech_security.md` §11.3 は要求値）だが `AuthApiTest` は 255 を正として書いてあるので該当テストを直す ②パスワード上限128の分岐（§11 #14「129文字以上」）に対応するテストが無い ③**メールアドレスの正規化（前後空白除去 + 小文字化）が `AuthServiceImpl` 未実装** — register の重複確認・login の検索・保存を正規化後の値へ寄せる（`known_issues.md` #21 は方針確定に伴い削除済み）。③は 3-A-2 修正セグメントA、①②は同セグメントC（`AuthApiTest`）で片付ける

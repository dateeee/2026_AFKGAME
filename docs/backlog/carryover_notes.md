...
- **`tech_logging.md` は残量12字**（2026-08-09。8,000字上限に対し 7,988字）。次に触るときは「バックエンドエラーハンドリング」（統一エラーレスポンス形式・エラーコード体系・AUTH_ コード一覧・グローバル例外ハンドラ ≒ 2,700字）を別ファイルへ切り出す。ログ設計とエラーハンドリングで2テーマ混在しており、`documentation_rules.md` §3.1 の緩和対象でもない

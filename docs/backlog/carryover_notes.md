...
- **Phase 3 の Python 実装（`c3e9a2b`「Phase 3 製造①」）がどのコードレビューも通っていない**（2026-08-08 に backend-review のスコープ確認で判明）。前回の全量レビューが 2026-08-04（`33ad1d0` の指摘反映まで）で、その後に入った `backend/app/{routers,services,models,schemas}/**` のパーティ・スキル分がレビュー未通過のまま。移行 STEP 5 で Java へ移植して破棄するなら不要だが、それまで Python が本番相当で動くなら1セッション要る（**ユーザー判断待ち**）

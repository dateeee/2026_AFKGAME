...
- **認証の入力長は仕様側を確定済み（2026-08-09・STEP 3-A-3）だが実装が未追随**。①メール長は **254**（`tech_auth/account.md` §9「入力長」が正。`tech_security.md` §11.3 は要求値）だが `AuthApiTest` は 255 を正として書いてあるので該当テストを直す ②パスワード上限128の分岐（§11 #14「129文字以上」）に対応するテストが無い。①②はどちらも Bean Validation 側なので 3-A-2 修正セグメントC（`AuthApiTest`）で片付ける。**③メールアドレスの正規化は 3-A-2 修正セグメントAで実装済み**（`AuthServiceImpl#normalizeEmail`。分岐 §11 #15・#16／§13 #14・#15 のテストも追加済み）

...
- **backend の Javadoc 2件が旧ドキュメントパスを指したまま**（2026-08-09 の配置換えで発生）。`JwtService.java` の `docs/process/coding_standards_backend/domain_service.md §2 #1` と `VerificationMailSender.java` の同 `§3 #8` を、いずれも `coding_standards_backend/domain/service.md` へ直す。配置換えは docs 領域の worktree で行い `backend/` を触らない方針にしたため未修正で残した（`check_docs.py` は Java を走査しないので**自動検出されない**）。**次に `backend/` を触るタスクのついでに直す**
- **`tech_logging.md` は残量41字**（2026-08-09。8,000字上限に対し 7,959字）。次に触るときは「バックエンドエラーハンドリング」（統一エラーレスポンス形式・エラーコード体系・AUTH_ コード一覧・グローバル例外ハンドラ ≒ 2,700字）を別ファイルへ切り出す。ログ設計とエラーハンドリングで2テーマ混在しており、`documentation_rules.md` §3.1 の緩和対象でもない

...
最終更新: 2026-08-09 / main `842023e`（**残っていた `親_子.md` 5組を精査し、親子3組を入れ子ディレクトリへ・対等2組を別名へ振り分けた**（ユーザー指示）。**先に欠陥を1件修正** — 直前の配置換えで `tech_forge_*.md` → `tech_forge/*.md` としたため `check_branch_list.py` の走査 `rglob("tech_*.md")` から3ファイルが外れ、**分岐一覧3件を一度も見ないまま「違反なし」を出していた**。走査を `rglob("*.md")` へ広げ（29件 → 32件）、キーを「層ディレクトリだけ落としたパス」（`tech_forge/craft.md`）へ変更。**移動**: `tech_auth_account.md` → `tech_auth/account.md`、`tech_api_common.md` → `tech_api/common.md`、`ui_onboarding.md` → `ui/onboarding.md`。**改名**（対等のため接頭辞を外した）: `tech_structure_backend.md` → `tech_backend.md`、`tech_operations_procedure.md` → `tech_maintenance.md`。参照元67ファイル（docs 33・`.claude/project` 2・**backend 30**）を更新し、`documentation_rules.md` は §6.1 の図解を兄弟方式へ差し替え・必須事項#8 を分岐マーカーの記法へ・§6.4「ファイル名の付け方」を新設（8,365字で上限超過のため `check_doc_size.py` の台帳へ分割方針つきで登録＝**台帳が空でないので Phase 完了ゲートの前に解消が要る**）。`carryover_notes.md` §3 の Javadoc 旧パス2件も解消済み。**backend はコメントのみの変更**で `mvn test` 166件 BUILD SUCCESS、コードは `d6eeca3`（Service のインタフェース分割）以降 実質無変更。直前の成果は**ログ3種別の方針確定とログ規約の3分冊化**で、**ドキュメントのみ・実装が未追従**（差分5点は同 §3）。直前までの成果は [changelog.md](../changelog.md) の 2026-08-09 ブロックが正。
...
前提: main の最新 `842023e`（前回レビューは `docs/reviews/backend-review/2026-08-08_221814.md`。以降の backend の差分は Red の 9647e06・3-A-2 の実装 41e92aa・**Service のインタフェース分割 d6eeca3**・ログ共通部品 09557ec。`64409f3` 以降はドキュメントが主で、`842023e` の backend 30ファイルは**Javadoc とテストの分岐マーカーのドキュメントパス更新のみ**＝ロジック無変更）。**読み取りのみなので worktree を作らない**（[worktree_guide.md](../process/worktree_guide.md) §5.1 #1）。重点は**上の最終更新に挙げた3つの判断** — `PasswordEncoder` の1本化、`VerificationMailSender` の仮実装、テスト2件の是正。`docs/backlog/open_specs.md` は**不在＝未確定ゼロ**
```

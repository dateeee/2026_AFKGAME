...
最終更新: 2026-08-12 / main `f04537f`（**3-B 製造①-ii：戦闘計算の Green**。戦闘計算6クラスの本体を実装して**対象29件を Green** にし `@Service` を付与、クリティカル率の供給元と `INTERNAL_MASTER_DATA_INVALID` を仕様へ確定した。詳細は [changelog.md](../changelog.md) の 2026-08-12 ブロック）。1つ前は main `b34543a`・`b9549f7`（フロント規約の整備と初期違反5件の known_issues 登録）、その前は `87e50f3`（製造①-i：表層27ファイル）。

**製造①-ii が残した前提**（①-iii・①-iv が上に積む）。

| # | 事実 |
|---|------|
| 1 | **戦闘計算6クラスは Green + `@Service` 済み**（`DamageCalculator`・`TargetSelector`・`StatCalculator`・`HealingCalculator`・`EncounterSelector`・`CharacterGrowth#addExp`）。残る4クラスは本体が `UnsupportedOperationException` のまま |
| 2 | **残る4クラスへ `@Service` を付けられるのはセグメント②**。`BattleSimulator` は `FloorProgression`・`Enemies`、`LapAnalyzer` は `FloorCatalog` を注入するため、実体が無いままスキャンに載せると**結合テスト85件のコンテキスト起動が壊れる**。Green の回では付けない |
...
| 4 | **クリティカル率の供給元は [tech_rng.md](../tech/detail/tech_rng.md) §6 が正**（味方 `character_types.yml`・敵 `EnemyData` の列）。ただし `BattleSimulatorImpl` は `CharacterTypes` を注入しておらず**テストが6引数のコンストラクタを直接呼ぶ**ため、味方側を配線する回はテストの生成箇所と申し送り Javadoc を同じ編集で直す |
| 5 | **`check_java_conventions.py` は 違反0 / WARN 13 が現在の正常値**（14 → −1 は `GameSettings#maxPlayerLevel()` が参照されたため）。`LogReason.CLOCK_SKEW` を出力する回でもう1件減る |
| 6 | 表層の正は各テストクラスの Javadoc「製造工程への申し送り」。**そこに無い名前を新設しない** |

**Java 移行の残りは 3-B（Phase 1: game / battle / tower）→ 4（Phase 2）→ 5（Phase 3）→ 6（切替と後始末）**（順序の正は [carryover_notes.md](carryover_notes.md) §1、手順・進捗の正は [java_migration.md](java_migration.md)、STEP の定義は [steps.md](java_migration/steps.md)「STEP 3〜5」）。**3-B は詳細設計・テストリスト作成①（game / battle）・製造①-i・①-ii まで完了**で、残るのは製造①-iii → ①-iv → テストリスト作成②（tower）→ 製造②。**Phase 1〜3 の機能はどの言語でも未実装の期間**（E2E はハーネスと `GET /health` まで疎通済みで、テスト本体は STEP 5 完了まで赤が正常）。

**Red の現況**: 単体378件のうち **Red 43件は `BattleService`・`BattleSimulator`・`OfflineCalculator`・`LapAnalyzer` の4クラスだけ**（72 → 43。surefire XML の実測）。既存の単体テストと**結合テスト85件は全件 green**で、**C1 は 100%（210/210・未達0）**。`-Dtest=<クラス>` で領域ごとに Green を取っていく。
...
```
/dev 3-B 製造①-iii（tick・ターン処理の Green）: `BattleSimulator` と `BattleService` を実装して Green にする
完了条件: ①`BattleSimulatorImplTest`（9件）・`BattleServiceImplTest`（12件）が全件 Green（`-Dtest=<クラス> -Dsurefire.failIfNoSpecifiedTests=false`） ②`PlayerRepository#findByIdForUpdate`／`#updateTickState` のマッピング XML をこの回で書く（インタフェースだけ先行済み） ③**味方のクリティカル率を配線する**（正は `tech_rng.md` §6。`character_types.yml` へ列を足し、`BattleSimulatorImpl` のコンストラクタと `BattleSimulatorImplTest` の生成箇所・申し送り Javadoc を同じ編集で更新する）。敵側は `EnemyData` の列（YAML はセグメント②） ④`LogReason.CLOCK_SKEW` を実際に出力する（巻き戻しの検知条件は `tech_tick.md` §1.1。WARN 13→12） ⑤**`@Service` は付けない**（前提2） ⑥`mvn verify -Dmaven.test.failure.ignore=true` で**結合テスト85件 green・C1 100%** のままを確認する ⑦`check_java_conventions.py` 違反0・`check_branch_list.py --tests` WARN 0 ⑧main へ統合してコミットする
参照: 上記2テストクラスの Javadoc「製造工程への申し送り」（表層の正）。ターン処理は [tech_battle.md](../tech/detail/tech_battle.md) §3.1・§5、tick の外枠は [tech_tick.md](../tech/detail/tech_tick.md) §1・§5、乱数源の生成点は [tech_rng.md](../tech/detail/tech_rng.md) §2
前提: main `f04537f`。**Maven 3.9.11 + JDK 17.0.20（Temurin）を新規シェルで `mvn -version` により実行確認済み**（`mvn verify` に Docker 不要。同セッションで実測）。**協調先の6クラスは Green 済み**で、`FloorProgression`・`Enemies` はテストがモックする（実体はセグメント②）。**worktree `3b-dev-tick` を作って作業する**（`python scripts/worktree.py add 3b-dev-tick`）。`docs/backlog/open_specs.md` は不在＝未確定ゼロ
```
...
|------|-------|------|------------|-----------|
| 1 | **3-B 製造①-iv（簡略計算の Green）**。`LapAnalyzer`（10件）・`OfflineCalculator`（12件）。依存はテストが全てモックするため `@Service` は付けない | §1（tick） | `3b-dev-offline`<br>backend | `dev` |
...
| 3 | **3-B 製造②（Phase 1: tower）**。`FloorCatalog`・`FloorProgression` の実装（`service/tower`）と `Enemies` のレジストリ化（`enemies.yml` + `@Component`）。**Green 済み4クラスへ `@Service` を付けるのもこの回**（前提2） | キュー2 | `3b-dev-tower`<br>backend | `dev` |
| 4 | **キャラ成長のテストリスト作成**（`applyLevelUp`・`addExp` のしきい値到達）。分岐は `tech_party.md` §6 の3件 + `tech_numeric.md` §5 へ足す到達側。成長率の列追加（`character_types.yml`・`CharacterTypeData`・`character.md` §1.2）を含む | 前提3 | `3b-testlist-growth`<br>backend | `test-list` |
...
- **`tech_polling.md` §5 の10件は `integration-test`（E2E）担当**（①-a で JUnit 対象外と判断）。tick API が要るため製造①-iii（§1）の後

...
最終更新: 2026-08-11 / main `33abd77`（**3-B 分岐一覧の差し戻し4件**＝ ①`tech_offline.md` §4.1 の期待値計算式を **§6「期待値計算式」10行**（Phase 1 で到達・全行に Red あり）と **§7「スキル/パッシブ依存」12行**（Phase 3〜・マーカーなし＝照合対象外）へ割り、`ticksPerLap = ceil(Σ撃破ターン数 ÷ turnsPerTick)` を明文化した ②`tech_tick.md` §5 #10（パーティが空）の `last_tick_at` を**「進める」に確定**し、同 §5 を標準形式4列へ移行した（**行は分割せず番号を保持**） ③`tech_numeric.md` §5 に #13・#14（回復量の下限1） ④`tech_rng.md` §5 に #9（生存者ありのターゲット抽選）。Red は `LapAnalyzerImplTest` を新規＋既存3クラスへ追記。`check_branch_list.py --tests` は exit 0・**WARN 0**（分岐一覧45件）、`mvn test-compile` は未実装型のシンボル未検出66件で失敗＝**期待どおりの Red**、`check_java_conventions.py` 違反0・WARN 13で横ばい、`check_docs.py`／`check_doc_size.py` 違反0、プロダクトコードの変更なし。詳細は [changelog.md](../changelog.md) の 2026-08-11 ブロック）。1つ前は main `280a1f4`（docs・`.claude` の整備）。

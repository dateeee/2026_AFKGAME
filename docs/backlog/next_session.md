...
最終更新: 2026-08-12 / main `2520410`（**3-B 製造①-iv：簡略計算の Green**。`LapAnalyzerImpl`・`OfflineCalculatorImpl` を実装して**対象22件を Green** にし、**単体378件の Red を0**にした。詳細は [changelog.md](../changelog.md) の 2026-08-12 ブロック）。1つ前は main `5d77dce`（製造①-iii：tick・ターン処理の Green）。
...
前提: main `2520410`。**Maven 3.9.11 + JDK 17.0.20（Temurin）を新規シェルで `mvn -version` により実行確認済み**（`mvn verify` に Docker 不要。同セッションで実測）。**単体378件・結合88件が全件 green（Red 0）・C1 100%（282/282）** が着手時点の実測値。`docs/backlog/open_specs.md` は不在＝未確定ゼロ。**worktree `3b-testlist-tower-a` を作って作業する**（`python scripts/worktree.py add 3b-testlist-tower-a`）
```

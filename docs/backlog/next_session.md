...
最終更新: 2026-08-10 / main `bd86a32`（**3-B: tower の詳細設計を完了**。候補キューの旧3を消化）。`/api/tower/*` 5API と tick 内の階進行を [tech_tower.md](../tech/detail/tech_tower.md)（索引）+ `tech_tower/` 4分冊へ確定し、分岐一覧55件を作成（check_docs・check_branch_list・check_doc_size 違反なし）。挙動は削除済み Python 参照実装（`git show 1819942^`）で裏取りし、**リタイア即時成立・HP閾値撤退の行き先（`auto_repeat` は塔内1階から再開）・`TOWER_` エラーコード統一**の3点で `battle.md`／`tech_state.md` の矛盾を解消した（内訳は [changelog.md](../changelog.md) の 2026-08-10 ブロックが正）。**3-A-2 の残レビュー指摘はセグメントE の3件（ISSUE-802・806・803）のまま**（レポートの正は [2026-08-10_013313.md](../reviews/backend-review/2026-08-10_013313.md)）。以下は1つ前の状態（**例外の gfw 標準化を完了**。`known_issues.md` #19・#21 を消化）。自作 `AppException` を廃止して `BusinessException` + `ResultMessages`（404 は `ResourceNotFoundException`）へ寄せ、**HTTP ステータスと応答文言は Web 層の `ErrorCatalog`（コード→ステータス対応表・既定 422）が持つ**形にした（応答 `message` の正は対応表側。仕様書は文言列を持たない）。あわせて #21 の3件（`details` 出力・400/422 の原因例外による切り分け・4xx のサニタイズ）と照合スクリプト `scripts/check_error_codes.py`（+ 回帰テスト）を入れた。`mvn verify` は **単体211件 + 結合58件 Green・C1 100%（144/144・未達0）**、`check_java_conventions.py` は113ファイル違反なし。**新規に `known_issues.md` #22 を登録** — `python -m pytest scripts/tests -q` が `check_branch_list.py` の回帰テスト30件で failed（着手前からで本体の出力は正常に見える。本体とテストのどちらが正かの確認が要る）。

**STEP 2R は完了済みで backend の Phase 機能へ着手してよい**。以後の移行順序は **3-A-2 → 3-A-3 → 3-B（Phase 1: game / battle / tower）→ 4（Phase 2）→ 5（Phase 3）→ 6（切替と後始末）**。順序の正は [carryover_notes.md](carryover_notes.md) §1、手順・進捗の正は [java_migration.md](java_migration.md)（索引 + `java_migration/` 3分冊）。**tower の詳細設計は完了**（[tech_tower.md](../tech/detail/tech_tower.md) 索引 + 4分冊）。3-B の残りはテストリスト作成 → 製造（下記「キューが空いたら戻す行」）。
...
- **キューが空いたら戻す行**: 3-A-3 の製造（分岐82件のためセグメント2本を見込む。`auth-3a3-dev-a` / `-b` / backend / `dev`。前提はキュー1・2 のテストリスト）→ 3-B のテストリスト作成 → 製造（Phase 1: game / battle → tower の順。tick・戦闘サービスが先に要るため tower のテストは後段。tower の分岐一覧は [tech_tower.md](../tech/detail/tech_tower.md) §0 の55件 + `tech_state.md` §5 の7件。順序の正は [carryover_notes.md](carryover_notes.md) §1）

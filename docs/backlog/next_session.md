...
最終更新: 2026-08-09 / main `5f4be91` + 本コミット（**ログ規約 `logging.md`（10,454字）を3種別ごとの3分冊へ分割した**（ユーザー指示 `/doc-size`）。`logging.md` = **索引 + 3種別共通**（§1 出力先・appender 割り当て / §5 エラーログ / §6 禁止事項 / §7 テストと分担・5,448字）、`logging_communication.md` = §2 通信ログ（2,026字）、`logging_application.md` = §3 AOP 境界ログ + §3.1 マスク + §4 業務ログ（4,682字）。`domain.md` / `domain_service.md` と同じ**兄弟ファイル方式**（入れ子ディレクトリを作らない）で、**節番号 §1〜§7 は3分冊で通しのまま維持**した（`tech_logging.md`・`common.md` からの既存参照を生かすため）。**本文の削除・要約はなく**、増分は分冊間の相互リンクのみ。参照元は規約索引 `coding_standards_backend.md`（10分冊→13分冊）・`spec_ownership.md` の2行・`common.md` §7・`tech_logging.md` の §2/§3/§4 を更新し、`tech_logging.md` は残量12字だったため重複する出力項目の列挙を落として 7,959字（残量41字）へ。直前の成果は**ログ3種別（通信 / アプリケーション / エラー）の方針確定**（`logging.md` 新設・`common.md` §7 の縮約・語彙＝仕様 / 出力先と書き方＝規約 の分担宣言）と `.claude/references/coding-standards-backend.md` の分割。**ログ3種別はドキュメントのみで実装が未追従**（差分5点＝appender 3本・`RequestLogFilter` の START・AOP 新設・`LoggerName`/`LogKey` の追加値・送信ログは [carryover_notes.md](carryover_notes.md) §3）。**backend のコードは `d6eeca3`（Service のインタフェース分割）以降 無変更**。直前までの成果は [changelog.md](../changelog.md) の 2026-08-09 ブロックが正。

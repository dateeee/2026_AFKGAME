...
最終更新: 2026-08-09 / main `9c73ab7` + 本コミット（**ログのコーディング規約を1分冊へ集約し、ログ3種別＝通信 / アプリケーション / エラーの方針を確定した**（ユーザー指示）。`docs/process/coding_standards_backend/logging.md` を新設して `common.md` §7 を3項目 + ポインタへ縮約し、`tech_logging.md` から「ログの書き方（共通部品）」の本体を移管して**語彙＝仕様 / 出力先と書き方＝規約**の分担を `spec_ownership.md` へ宣言した。**決めた3点**（ユーザー選択）— AOP（Service・Repository 境界）は**引数・戻り値まで INFO で常時出力**／出力先は `LOG_DIR` 配下に**日次ローテ・保持14日**／通信ログは**受信＋送信**の両方。**ドキュメントのみで実装は未追従**（差分5点＝appender 3本・`RequestLogFilter` の START・AOP 新設・`LoggerName`/`LogKey` の追加値・送信ログは [carryover_notes.md](carryover_notes.md) §3）。直前の成果は `.claude/references/coding-standards-backend.md` の索引 + 分冊3件への分割（`/doc-size`）と、前セッション `09557ec` のステージ漏れ取り込み（`3bed24b`〜`eb574c5`）。**backend のコードは `d6eeca3`（Service のインタフェース分割）以降 無変更**。直前までの成果は [changelog.md](../changelog.md) の 2026-08-09 ブロックが正。
...
- **キューが空いたら戻す行**: 3-B の残り（Phase 1: game / battle）と、それぞれのテストリスト作成 → 製造（順序の正は [carryover_notes.md](carryover_notes.md) §1）／**ログ3種別の実装**（規約は `coding_standards_backend/logging.md`、実装との差分5点は [carryover_notes.md](carryover_notes.md) §3。`java_migration.md` の STEP と独立に着手できる。wt 名 `logging-3types` / backend・工程スキル `dev`）

...
最終更新: 2026-08-09 / main `e3b5dd4`（**索引に子を持つ分冊を「兄弟ファイル方式（`親_子.md`）」から「入れ子ディレクトリ方式（`親/子.md`）」へ配置換えした**（ユーザー指示）。`coding_standards_backend/{domain/service, logging/communication, logging/application}.md` と `tech/detail/tech_forge/{craft, disassemble, enhance}.md` へ `git mv` し、参照元18ファイル（規約索引・分冊9件・`phases.md`・`tech_logging.md`・`spec_ownership.md`・`tech_forge.md`・`tech_spec.md`・`tech_base.md`・`tech_rng.md`・`data/master/equipment.md`・`.claude/project/detail-design.md`）を更新。**節番号・本文は据え置き**で、`check_docs.py` 違反なし・`check_doc_size.py` 0違反。**backend の Javadoc 2件（`JwtService`・`VerificationMailSender`）の旧パス参照だけ未修正**＝領域が異なるため [carryover_notes.md](carryover_notes.md) §3 へ申し送り済み。直前の成果は**ログ3種別（通信 / アプリケーション / エラー）の方針確定とログ規約の3分冊化**で、**ドキュメントのみ・実装が未追従**（差分5点＝appender 3本・`RequestLogFilter` の START・AOP 新設・`LoggerName`/`LogKey` の追加値・送信ログも同 §3）。**backend のコードは `d6eeca3`（Service のインタフェース分割）以降 無変更**。直前までの成果は [changelog.md](../changelog.md) の 2026-08-09 ブロックが正。

...
最終更新: 2026-08-16 / main `3ca8262`（**diagrams-review 還元案3件の適用**。`check_mermaid.py` の新設・`check_schema_triple.py` への `--note`（備考の記録時点）追加・`fix-specs` のルール9「図への波及を確認する」。詳細は [changelog.md](../changelog.md) の 2026-08-16 ブロック）。1つ前は main `2520410`（**3-B 製造①-iv：簡略計算の Green**。`LapAnalyzerImpl`・`OfflineCalculatorImpl` を実装して**対象22件を Green** にし、**単体378件の Red を0**にした）。

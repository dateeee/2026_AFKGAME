...
最終更新: 2026-08-09 / `156126f` で **`wt/drop-python-backend` を main へ統合**した（Python/FastAPI 削除、移行 STEP 6 の1〜3が完了）。あわせて `.vscode/launch.json` を Java 構成へ差し替え、ER図に `characters.master_id` を足して `check_schema_triple.py` を exit 0 へ戻した。その後 main で**バックエンドコーディング規約の整備**を続け、`basis.md` §3 の逸脱一覧（17件）を削除して**ガイドラインとの差分の正を各分冊へ一本化**した。さらに `4411b42` で `wt/repo-abstraction` を統合し、**データアクセスを Repository で抽象化する方針へ改訂**した（layering.md §3 の「Repository を作らない構成」を廃止。実装も Mapper 8件 → Repository 5件へ再編し `mvn clean verify` PASS。要点は [carryover_notes.md](carryover_notes.md) §2）。**§1 のタスクは未着手のまま**。
...
前提: main の最新（`75038b1`。規約は差分一覧の一本化・Repository 化に加え、**例外規約の3分冊化＝`coding_standards_backend/exception.md`（ビジネス例外・システム例外・予期しないエラーの3分類）**まで済んでいる。`SystemException` は規約のみで実装は未着手。派生の `.claude/references/coding-standards-backend.md` は 4,994字 / 上限5,000字で、次に追記するなら先に圧縮か分割が要る）。**main には untracked が2件残っている** — `backend/afkgame.db`・`backend/e2e.db` は Python 時代の残骸で、**削除が権限分類器に拒否されて未実施**（同じ理由で `backend/{app,tests,alembic,htmlcov,.pytest_cache,.ruff_cache,.env,.github}` と `__pycache__` 18件も物理的に残っている。いずれも git 管理外で中身は `.pyc` とキャッシュのみ）。**着手をためらう必要はない**が、消すならユーザーの承認を取ってから `rm -rf` する。**ドキュメントのみを編集するので worktree を作る**: `python scripts/worktree.py add p4limitbreak-detail` → `EnterWorktree` に `path` で移動（領域は docs/tech。§2 の他行と重ならない）。**本タスクは Java を使わない**。必要になった場合の実行形は `JAVA_HOME="/c/Program Files/Eclipse Adoptium/jdk-17.0.20.8-hotspot" "/c/Users/tubas/AppData/Local/Programs/apache-maven-3.9.11/bin/mvn" -f backend/pom.xml clean verify`（2026-08-09 に新規シェルで `mvn -v` 実行確認済み。`mvn`・`java` は PATH にも JAVA_HOME にも無い。出力は CP932 なのでログはファイルへ落として `iconv -f CP932 -t UTF-8` で読む。**`clean` を外すと増分ビルドで `afkgame-initdb` が落ちる** — 理由は carryover_notes.md §4）
```

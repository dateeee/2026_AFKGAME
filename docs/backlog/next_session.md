...
最終更新: 2026-08-09 / `ac92ef7` で **移行 STEP 2R-0（先行検証）を完了**した。不確定要素6件を実機で確定している（Tomcat は **11.0（Servlet 6.1）を標準**・10.1 でも動作／埋め込み PostgreSQL は `EmbeddedPostgres.builder().start()` の直接起動／Flyway は `@Bean(initMethod = "migrate")` ＋ `@DependsOn`／`/health` の version は **Maven のリソースフィルタ**／ビルドプラグインの競合なし（ただし雛形の failsafe は `verify` ゴールを持たない）／プロファイル切替は **環境変数のみ**）。**確定内容と根拠の正は [java_migration/steps.md](java_migration/steps.md) §4「2R-0 の確定結果」**。STEP 2R は「着手中」となり、次は **2R-A**。
...
```
/basic-design 移行 STEP 2R-A（仕様書・規約の再改訂）: 2R-0 で確定した6件と [tech_selection.md](java_migration/tech_selection.md) §2 の変更点を仕様書・規約へ反映し、2R-B 以降が仕様を見ながら組める状態にする
完了条件: 反映後に `python scripts/check_doc_size.py` と `python scripts/check_docs.py` を exit 0。反映漏れの確認は `grep -rn "spring-boot\|application\.yml\|application-local\|@ConfigurationProperties\|logback-spring" docs/tech docs/process` の残存箇所がすべて説明のつくものになっていること
参照: **反映元の正は [java_migration/steps.md](java_migration/steps.md) §4「2R-0 の確定結果」**（6項目の方式と根拠。実在を確認済み）と [tech_selection.md](java_migration/tech_selection.md) §2。反映先は [tech_structure.md](../tech/basic/tech_structure.md) §2〜§4（配置・設定値）、[tech_operations.md](../tech/nonfunctional/tech_operations.md) §12（環境・起動・デプロイ・環境変数の表）、[coding_standards_backend.md](../process/coding_standards_backend.md)（索引 → 該当分冊のみ）。**Jackson 3 と 2 の同居**はここでは決めない（2R-C の決定事項として tech_selection.md §2 に記載済み）
前提: main の最新 `ac92ef7`（STEP 2R は「着手中」。2R-0 完了、次が本タスク）。**反映先2ファイルは残量が少ない** — `tech_structure.md` 7,671字（残り329字）・`tech_operations.md` 7,319字（残り681字）。**書く前に `python scripts/check_doc_size.py --sections <path>` で測り**、超えるなら既存記述の圧縮か分割を同じ編集にまとめる。**ドキュメントのみを編集するので worktree を作る**: `python scripts/worktree.py add tera-2ra` → `EnterWorktree` に `path` で移動（領域は docs/tech・docs/process。§2 の他行と重ならない）。**本タスクは Java を使わない**。必要になった場合の実行形は `JAVA_HOME="/c/Program Files/Eclipse Adoptium/jdk-17.0.20.8-hotspot" "/c/Users/tubas/AppData/Local/Programs/apache-maven-3.9.11/bin/mvn" -f backend/pom.xml clean verify`（2026-08-09 に新規シェルで実行確認済み。`mvn`・`java` は PATH にも JAVA_HOME にも無い。出力は CP932 なのでログはファイルへ落として `iconv -f CP932 -t UTF-8` で読む。`clean` を外すと `afkgame-initdb` が落ちる — 理由は carryover_notes.md §4）。**Docker Desktop のデーモンは停止している**（`docker` CLI 29.6.2 はある）。`mvn verify` は埋め込み PostgreSQL なので Docker 不要だが、`local` の DB を上げる作業には起動が要る。**main には untracked の残骸がある** — `backend/afkgame.db`・`backend/e2e.db` ほか Python 時代のディレクトリ（`.gitignore` 済みで `git status` には出ない）。消すならユーザーの承認を取ってから `rm -rf` する
```
...
|------|-------|------|------------|-----------|
| 0 | **移行 STEP 2R-B（REST 専用の土台を作る）**。Archetype `5.11.0.RELEASE` で雛形を生成し、Thymeleaf と View 一式・`-selenium` を落として `mvn clean install` で war が出るまで持っていく。**雛形の failsafe に `verify` ゴールを足す**（2R-0 の確定結果） | **2R-A 完了後**。生成コマンドと方式は [steps.md](java_migration/steps.md) §4 | `tera-2rb`<br>backend | `dev` |

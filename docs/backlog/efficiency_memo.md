...
## 2026-08-08 18:11 | session 35674a86 | 自動検出
- シグナル: long-turn(calls=31)
- ターン概要: ツール31回・エラー0回・拒否0回。開始:「もう少しworktreeを使いやすくしたい。」
- 原因と改善案: ほぼ誤検出（スクリプト改修＋ガイド改訂＋スモークテスト＋worktree 統合まで1タスク完遂の工程相応。エラー0・拒否0）。唯一の無駄は main 側で調査 Read した worktree.py / worktree_guide.md を worktree 入り後に Edit 前提でもう一度 Read した二重読み込みで、worktree_guide §5.2 に「編集対象が特定済みなら worktree へ入ってから読む」を追記すると防げる。

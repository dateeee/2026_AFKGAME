...
| 28 | `frontend/src/types/game.ts`（`PlayerState.gold`）・ゲーム状態／ショップの Resource（未実装） | **所持ゴールドの JSON 表現が仕様と実装で食い違う**。[tech_data.md](../tech/basic/tech_data.md) §1.1 で「JS の安全整数（2^53-1）を超え得る値は JSON 上で文字列」と規約化したが、`game.ts` の `gold` は `number` のまま。**Phase 1〜3 の API 自体が未実装のため現時点の実害は無い**（フロント単体の型宣言だけが先行している状態）。`GET /api/game/state`・`POST /api/shop/buy` を実装する回で、Resource 側の型・`game.ts` の `string` 化・表示側の短縮表記（K/M/B/T/Qa/Qi）の変換を**同じ回でそろえる**。tick報酬の `gold` / `gold_lost` / `totalGold` は1回ぶんの増減で安全整数に収まるため対象外 | 中（仕様と実装の不一致。Phase 1 の製造前に解消する） | spot-review ISSUE-1502 の反映で派生 |

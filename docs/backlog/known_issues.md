...
| 19 | `afkgame-domain`（`exception`・`service`）, `afkgame-web`（`filter`） | 例外規約を `terasoluna-gfw` 標準へ改訂したが（[exception.md](../process/coding_standards_backend/exception.md) §2.1）、実装は自作 `AppException(code, message, status)` のまま。①`AppException` を廃止して `BusinessException` + `ResultMessages`（404 は `ResourceNotFoundException`）へ置換（`AuthService`・`JwtService`・`ApiExceptionHandler`・`JwtAuthenticationFilter`・`ApiAuthenticationEntryPoint` + テスト6件）②Web層へコード→ステータス対応表を新設（`tech_error_handling.md` の15コードが正。既定 422）③照合スクリプト `scripts/check_error_codes.py` が**未作成**（規約 §4 #5 が要求）。**`AuthService` が例外へ日本語の文言を直接埋めており規約 §3 #5 にも反する**（gfw 化で同時に解消する） | 中（規約と実装の乖離。応答仕様は変わらない） | 規約整備 |
| 20 | `afkgame-web`（`web.xml`・`SpringMvcConfig`・`ApplicationContextConfig`・`RequestLogFilter`） | 横断処理の規約（[filter.md](../process/coding_standards_backend/filter.md) §5・[interceptor.md](../process/coding_standards_backend/interceptor.md) §4）を新設したが、雛形の gfw ロギング部品が残っている。不採用としたのは①`exceptionLoggingFilter`②`traceLoggingInterceptor`③`handlerExceptionResolverLoggingInterceptor` + `Advisor` の3件（①③は `ExceptionLogger` 依存で [exception.md](../process/coding_standards_backend/exception.md) §5 #3 に反し、コードも `e.xx.fw.*` で体系外。②は `RequestLogFilter` と二重計測）。連鎖で `exceptionLogger`・`exceptionCodeResolver` Bean も不要。④除去すると**フィルタ内の予期しない例外が記録されなくなる**ため、`RequestLogFilter` へ捕捉 + ERROR + 500（統一形式）を追加する（`filter.md` §4 #4。現在は再送出のみ）⑤`userIdMDCPutFilter`（`SpringSecurityConfig`）も不採用に加える。MDC キー `USER` は `LogKey` 体系（snake_case）の外で、principal が `UserDetails` でないため値は `com.afkgame.domain.model.User@…`。認証済みユーザーIDは `JwtAuthenticationFilter` が `player_id` として載せており重複する（Bean・`addFilterAfter`・import を削除。検証しているテストは無い） | 中（規約と実装の乖離。①〜③⑤は挙動据え置き、④は穴塞ぎ） | 規約整備 |
| 21 | 認証（`AuthServiceImpl`・`UserRepository`・`users.email`） | **メールアドレスの大文字小文字が正規化されていない**。登録の重複確認（§10 手順2）もログインの検索（§12 手順2）も生の値で等価比較し、`uq_users_email` も大小を区別するため、`User@Example.com` と `user@example.com` で別アカウントが作れて片方でログインできない。[account.md](../tech/detail/tech_auth/account.md) §9〜§12 と [tech_db/auth.md](../tech/basic/tech_db/auth.md) のいずれにも正規化の規約が無い。**アプリ層で小文字化するか、DB を `citext` / `lower(email)` の一意インデックスにするかの方針決定が先**（決めたうえで §9 の共通規約へ明記する）。既存データが無い今のうちが最も安い | 中（同一アドレスで重複登録。3-A-3 の確認メール再送・パスワード再設定も同じ経路） | backend-review ISSUE-703 |

...
| 21 | `afkgame-web`（`ApiExceptionHandler`） | REST ガイドライン 5.1 との照合で追加した規約3件が未実装。①`details`（違反項目）: `validationError()` が `MethodArgumentNotValidException#getBindingResult()` を捨てて定型文だけを返すため、`register` のように制約が複数ある API で**どの項目が落ちたかクライアントが判別できない**（[exception.md](../process/coding_standards_backend/exception.md) §4 #9・形式は [tech_error_handling.md](../tech/basic/tech_error_handling.md)）②400/422 の切り分け: `handleHttpMessageNotReadable` が構文破損した JSON も 422 で返す。原因例外（`JsonParseException`）で 400 へ分ける（規約 §4 #10）③4xx のサニタイズ: `handleExceptionInternal` が 4xx で `ex.getMessage()` を素通しし、`MethodArgumentTypeMismatchException` 等が**変換先のクラス名を応答へ載せる**（規約 §4 #1）。現在パス変数・クエリパラメータを持つ API が無いため③は潜在。**#19（`AppException` → gfw 化）と同じクラスを触るので同時に直す** | 中（①はUX・③は情報漏れ。②③は現時点で潜在） | REST ガイドライン照合 |

...
> 親: [battle_flow.md](../battle_flow.md)。仕様は [systems/battle.md](../../design/systems/battle.md)。
...
- 全滅ペナルティのうち「塔内取得アイテム全ロスト」は**現行実装に未反映**（ドロップ装備が即時永続化され取り消し対象として管理されていないため）。図・仕様が正で、実装側の是正は [known_issues.md](../../backlog/known_issues.md) で追跡する

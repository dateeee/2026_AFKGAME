...
最終更新: 2026-08-06 / 対応コミット: df85b3f の直後（スキル全体リファクタ Stage 2 のコミット）
...
前提: Phase 3 仕様確定ゲート完了（doc-review 6件を fix-specs で反映）＋スキルリファクタ Stage 2 完了（13項適用・コミット `df85b3f` の次）。分岐一覧の WARN 16件（例外経路の片側欠落の注記・ループの0周/1周/2周行・tech_shop 旧形式5件の標準形式移行）は本工程で解消する
```
...
| 2 | 逼迫3ファイルの圧縮（CLAUDE.md 残6字・tech_structure.md / tech_shop.md 残3字） | —（改稿時に随時） |
| 3 | スキルリファクタ Stage 3（`.claude/scripts/review_prep.py` の新設と効果検証。任意） | —（`docs/reviews/skill-improvement/2026-08-06_skills-refactor.md` §6 を指定） |

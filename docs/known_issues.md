...
| 9 | `tests/e2e/tower.spec.ts` | 「未解放の塔は選べず解放条件が表示される」が失敗する。未解放カードの `aria-disabled="true"` を Playwright が disabled と判定し、`locked.click()` が15秒でタイムアウトする。**製造完了ゲートの修正前から再現する既存の失敗**（修正を stash して確認済み）。テスト側を `dispatchEvent` 等へ寄せるか、カードの無効化表現を見直すかの判断が必要 | 中（L2が1件赤） | 結合テスト |

 ...
 * {@link TowerData#modifiers()} の要素として起動時に検証する（{@code @Valid} でカスケードする）。
 *
 * <p>保持するのは階進行が読む3項目（{@code type} / {@code trigger} / {@code value}）だけで、
 ...
 */
public record TowerModifier(@NotBlank String id, @NotBlank String type, @NotBlank String trigger,
        double value) {
}

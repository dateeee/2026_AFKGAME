...
| 図 | 内容 | ファイル |
|----|------|---------|
| 全体アーキテクチャ | フロント（Vue 3 SPA）・バックエンド（FastAPI）・DB のモジュール構成と依存関係 | [system_architecture/application.md](system_architecture/application.md) |
...
| 本番構成（AWS） | S3/CloudFront + EC2（Nginx・uvicorn・EBS・cron）のデプロイ構成とバックアップ経路 | [system_architecture/deployment.md](system_architecture/deployment.md) |

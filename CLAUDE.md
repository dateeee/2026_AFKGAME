...
| [docs/tech/tech_spec.md](docs/tech/tech_spec.md) | `tech/tech_*.md` — data / structure / api / architecture / logging（詳細設計: battle / offline / tick / polling / state / rng / numeric / shop / design_system、認証: auth） |
...
- 工程の区切り（レビュー完了・コミット後）では `/clear` を既定として提案する（同一タスクを続ける場合のみ `/compact`）。レビュー→修正適用（doc-review → fix-specs）は別セッションに分ける
